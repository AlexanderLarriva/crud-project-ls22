...
def test_login_pass():
    with requests.Session() as s:
        ...
        response = s.post(urljoin(BASE_URL, '/session/new'),
                          data=user, allow_redirects=False)
...
def test_post_session():
    with requests.Session() as s:
        ...
        response = s.post(urljoin(BASE_URL, '/session/delete'),
...
def test_delete_session():
    with requests.Session() as s:
        ...
        response = s.delete(urljoin(BASE_URL, '/session/delete'),
                            allow_redirects=False)
        response = s.get(urljoin(BASE_URL, '/'))
        assert user['name'] not in response.text
